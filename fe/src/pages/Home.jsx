    ...
    <div className={`h-screen w-full bg-black flex flex-col gap-y-[50px] items-center relative`}>
      ...
      <div className={`w-full h-24 bg-gradient-to-r from-cyan-600 to-blue-600 font-bold text-white text-2xl flex justify-between items-center px-40 py-8 transition-all ease-out duration-700 ${isVisible ? ' opacity-0 -translate-y-full' : 'opacity-100 translate-y-0 '}`}>
        ...
        <div className="flex gap-x-10 ">
          <h2 onClick={() => {setIsVisible(true); setIsLogin(true) }} className='cursor-pointer border-2 rounded-xl px-8 py-2 transition-all duration-500 hover:scale-105 active:scale-95'>Login</h2>
          <h2 onClick={() => {setIsVisible(true); setIsLogin(false) }} className='cursor-pointer border-2 rounded-xl px-8 py-2 transition-all duration-500 hover:scale-105 active:scale-95 '>Register</h2>
        </div>
      ...
      <div className={`w-[80%] h-auto flex flex-col gap-20 transition-all ease-out duration-1000 ${isVisible ? 'opacity-0 z-0 -translate-x-full' : 'opacity-100 '}`}>
        <div className='w-full h-[550px] flex justify-between'>
          ...
          <div className="h-full w-[500px]  flex flex-col justify-between text-white text-xl font-semibold ">
            {image.slice(0, 2).map(obj => (
                <div className='w-full h-[250px] border-4 border-cyan-300 rounded-3xl overflow-hidden relative'>
        ...
        {/* img 4 5 6 7 */}
        <div className="w-full h-[220px] flex justify-between text-white text-xl font-semibold">
          {image.slice(2, 6).map(obj => (
                  <div className='w-[350px] h-full border-4 border-cyan-300 rounded-3xl overflow-hidden relative'>
                  <img src={obj.src} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/60 opacity-0 hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                      {obj.label}
                  </div>
