...
export const DishDeleteForm = ({editId, setOpenDelete, setNotification, updateDishList}) => {

    const handleSubmit = async (e)   => {
            e.preventDefault();
            try {
                const response = await deleteDishById(editId)
                updateDishList();
                setOpenDelete(false)
    ...
    return (
        <div className='w-[400px] h-auto rounded-3xl bg-gray-100 p-6 space-y-6 shadow-lg '>
            <div className='flex justify-between items-center text-xl font-bold'>
                <h2 className='text-blue-500'>Delete dish</h2>
                <button onClick={()=>setOpenDelete(false)} className='text-gray-500 hover:text-red-600 cursor-pointer'>&#10006;</button>
            </div>

            <div className="flex flex-col items-center gap-5">
                <h2 className="font-bold text-center text-lg"> Do you want to delete this dish? </h2>
