...
export const CashierTableCard = ({ tb, setNotification, updateTableList }) => {
    ...
    return (
        ...
        ${tb.status === "available" ? " border-white" : "border-white/20"}`}>
            ...
            <h2 className={`text-center text-2xl ${tb.status === "available" ? "text-green-500" : "text-red-500"}`}>{tb.status}</h2>
            ...
            {tb.status === "occupied" && (
                <div className="absolute inset-0 bg-black/90 rounded-3xl flex items-center justify-center">
                    ...
                    onClick={()=>{setOpenPayment(true); setSelectedTable(tb)}}
                        className="relative rounded-xl border-2 border-blue-500 hover:cursor-pointer
